...
class Main:
    ...
    def __handle_row(self, rows: 'pd.DataFrame', database: 'pd.DataFrame'):
        ...
        if old_row.empty or len(old_row) > 1:
            raise Exception('Old row is empty or more than one')
        ...
        return self.__component.handle_new_line(database, old_row, new_row, new_lineid)
    ...
    def __handle_updated_rows(self, updated_rows: 'pd.DataFrame', database: 'pd.DataFrame'):
        ...
        for bundle in bundles:
            ...
            print(database.loc[database.index.isin(bundle.index)])
    ...
    def run(self):
        ...
        new_lines, updated_lines, inactivated_lines = self.__get_update_types(
            excel)
        table = self.__handle_updated_rows(updated_lines, database)
